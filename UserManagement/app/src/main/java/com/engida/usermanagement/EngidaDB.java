...
public class EngidaDB extends SQLiteOpenHelper {
  Context context;
...
//    private static final String con_6 = "sex";
    ...
    public EngidaDB(Context context) {
...
this.context=context;
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        try {
            sqLiteDatabase.execSQL("create table usertable ( fullnames text,usernames text,emails text,passwords text,mobiles text,sexs text )");
        } catch (Exception ex) {
            Log.e("EngidaDB", "Table creation Erro", ex);
        }

    }
    public void addMember(ModelClass regionModel) {
            SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
            ContentValues values = new ContentValues();
            values.put("fullnames", regionModel.getFname());
    ...
    public List<ModelClass> getAllUsers() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ModelClass users = new ModelClass();
